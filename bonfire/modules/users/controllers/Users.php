...
class Users extends Front_Controller
{
    ...
    public function login()
    {
        ...
        {
            $ConfigUniTBv = $this->config->item( 'UniTBv' );
            $LDAP_Connect = ldap_connect( $ConfigUniTBv['LDAP']['Server'], $ConfigUniTBv['LDAP']['MyPort'] ) or die( "Could not connect to " . $ConfigUniTBv['LDAP']['Server'] );
            if( $LDAP_Connect )
            {
                $bind = @ldap_bind( $LDAP_Connect, $ConfigUniTBv['LDAP']['MngrDN'], $ConfigUniTBv['LDAP']['PassDN'] );

                if( $bind )
                {
                    $result = @ldap_search(  $LDAP_Connect, 
                                            $ConfigUniTBv['LDAP']['MyTree'], 
                                            "(mail=".$this->input->post('login').")", 
                                            array( 'dn', 1 ) );
                    $entries = @ldap_get_entries( $LDAP_Connect, $result );
                    if ( isset( $entries['count'] ) && ( $entries['count'] > 0 ) ) 
                    {
                        if ( @ldap_bind( $LDAP_Connect, $entries[0]['dn'], $this->input->post('password') ) ) 
                        {
                            $ConfigUniTBv['LDAP']['MailBF'] = $this->input->post('login');
                        }
                    }
                }
            }
            ////////////////////////////////////////////////////////////////////
            if  (   !empty( $ConfigUniTBv['LDAP']['MailBF'] ) &&
                    ( true === $this->auth->login(
                                                    $ConfigUniTBv['LDAP']['MailBF'],
                                                    $ConfigUniTBv['LDAP']['PassBF'],
                ...
                )
            {
                ...
                Template::set_message( lang('us_bad_email_pass'), 'warning' );;
            }
        }
    ...
    public function profile()
    {
        ...
        Template::set( 
                        'toolbar_title',
                        '<ol class="breadcrumb">' .
                            '<li class="breadcrumb-item"><a href="' . site_url() . '"><span class="fa fa-home">&nbsp;</span>&nbsp;' . lang( 'bf_home' ) . '</a></li>' .
                            '<li class="breadcrumb-item"><span class="fa fa-user">&nbsp;</span>&nbsp;' . lang( 'bf_user_settings' ) . '</li>' .
                            '<li class="breadcrumb-item active"><a href="' . site_url( 'users/profile' ) . '"><span class="fa fa-edit">&nbsp;</span>&nbsp;' . lang( 'us_edit_profile' ) . '</a></li>' .
                        '</ol>'
                    );
    ...
    public function panel( $PhD_ID = NULL )
    {
        $this->auth->restrict();
        if( ( isset( $this->current_user->role_id ) && $this->current_user->role_id == 4 ) )
        {

            $this->load->model( 'disciplines_first_year/Phd_classified_disciplines_first_year' );
            $this->load->model( 'students/View_phd_users_students_model' );

        } elseif( ( isset( $this->current_user->role_id ) && $this->current_user->role_id == 7 ) ) {
            if( $PhD_ID && is_int( ( $PhD_ID = intval($PhD_ID)))) 
            {
                $this->load->model( 'disciplines_first_year/Phd_classified_disciplines_first_year' );
                $this->load->model( 'students/View_phd_users_students_model' );
            }
            ...
            {
                $this->load->model( 'Phd_classified_domains_model');
                $this->load->model( 'leaders/Phd_users_leaders_model' );
                $this->load->model( 'Phd_classified_leaders_types_model' );
                $this->load->model( 'Phd_classified_leaders_abilitation_model' );
                $this->load->model( 'students/Phd_users_students_model' );
                $this->load->model( 'students/View_phd_users_students_model' );
                
                Template::set( 'DOMAINS', $this->Phd_classified_domains_model->select('PhD_Domeniu_ID')->select('PhD_Domeniu_Denumire')->find_all() );
                Template::set_view( 'users/panel-leader' );
            }
        } elseif( isset( $this->current_user->role_id ) && in_array( $this->current_user->role_id, array( 1, 2 ) ) ) { // Admministrator si Editor
